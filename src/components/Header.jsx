...
const Header = () => {
    ...
    const linkClass = ({ isActive }) =>
        `w-fit flex items-center px-1 py-1 rounded-md transition-colors flex-none ${isActive
            ? "text-rose-500 before:content-['>'] before:mr-1"
            : "text-white hover:text-rose-500"
        }`;


    const startsWithLinkClass = (pathPrefix) => ({ isActive }) => {
        
        const isCurrentRouteActive = isActive || location.pathname.startsWith(pathPrefix);
    ...
    return (
        <nav className="fixed top-0 left-0 w-full z-50 bg-gray-900/95 backdrop-blur-sm border-b text-md md:text-sm lg:text-base border-gray-700 font-mono"> {/* Removed h-16 and flex items-center from here */}
