          ...
          .map((comment) => (
            <div key={comment.id || "temp"} className="group relative">
              <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 hover:border-rose-100 transition-colors">
                ...
                <CommentReportModal commentId={comment.id} userId={userUid} listId={listId} />
                      ...
                      {commentTree[comment.id] && (
                        ...
                        >
                          {showReplies[comment.id]
                            ? "Hide replies"
                            : `View ${commentTree[comment.id].length} ${commentTree[comment.id].length === 1 ? "reply" : "replies"}`}
                        </button>
                      )}
                    </div>
                  </div>
              ...
              {showReplies[comment.id] && commentTree[comment.id] && (
                <div className="mt-4 ml-8 pl-4 border-l-2 border-gray-100 space-y-4">
                  {commentTree[comment.id].map((reply) => (
                    <div key={reply.id} className="bg-gray-50 p-4 rounded-lg border border-gray-100">
                      <div className="flex items-start gap-3">
                        <div className="flex-shrink-0 w-6 h-6 rounded-full bg-rose-100 flex items-center justify-center">
                          {userData[reply.userId]?.photoURL ? (
                  ...
                  className="absolute top-2 right-2 p-1 bg-red-500 text-center rounded-md hover:bg-red-600 text-gray-500 hover:text-gray-700 opacity- group-hover:opacity-100 transition-opacity"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-exclamation text-slate-100" viewBox="0 0 16 16">
